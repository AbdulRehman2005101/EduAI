...
const CoursePage = () => {
  const user = mockUser; // Replace with your auth context
  ...
  const [personToAdd, setPersonToAdd] = useState({ email: "", role: "STUDENT" });

  // Permission checks
  const isTeacher = user?.role === "TEACHER" && course?.teacher.email === user.email;
  const isTA = user?.role === "TA" && course?.tas.some(ta => ta.email === user.email);
  const canManageContent = isTeacher || isTA;
  const canManagePeople = isTeacher; // Only teachers can manage people
  ...
  const fetchAnnouncements = async () => {
    try {
      const response = await fetch(`/api/course/${courseId}/announcements`);
      if (response.ok) {
  ...
  const fetchLectures = async () => {
    try {
      const response = await fetch(`/api/course/${courseId}/lectures`);
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchChatMessages = async () => {
    try {
      const response = await fetch(`/api/course/${courseId}/chat`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleCreateAnnouncement = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Error creating announcement:", data.error);
  ...
  const handleCreateLecture = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Error creating lecture:", data.error);
  ...
  const handleAddPerson = async () => {
    ...
    try {
      const response = await fetch(`/api/course/${courseId}/people`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (response.ok) {
        await fetchCourseData(); // Refresh course data to update people list
        setPersonToAdd({ email: "", role: "STUDENT" });
        setShowAddPersonDialog(false);
        alert(`Successfully added ${personToAdd.email} as ${personToAdd.role.toLowerCase()}`);
      } else {
  ...
  const handleRemovePerson = async (userId: string, personName: string) => {
    if (!confirm(`Are you sure you want to remove ${personName} from this course?`)) {
      return;
    }

    try {
      const response = await fetch(`/api/course/${courseId}/people/${userId}?requesterEmail=${encodeURIComponent(user?.email || '')}`, {
        method: "DELETE"
      });
      ...
      if (response.ok) {
        ...
        console.error("Error removing person:", data.error);
  ...
  return (
    <div className="flex-1 min-h-screen w-full p-6 bg-gradient-to-br from-[#1B2023] to-[#2A2F35]">
      <DashboardHeader 
        ...
        subtitle={`Class Code: ${course?.classCode || "N/A"}`} 
      />
        ...
        <TabsContent value="announcements" className="space-y-4">
          ...
          <div className="space-y-4">
            {announcements.map((announcement) => (
              <div key={announcement.id} className="bg-gray-800 border border-gray-700 rounded-lg p-6">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-xl font-semibold text-white">{announcement.title}</h3>
                  <div className="flex items-center gap-2">
                    <Badge variant="secondary">{announcement.author.role}</Badge>
                    {canManageContent && (
        ...
        <TabsContent value="assignments" className="space-y-4">
                ...
                <DialogContent className="bg-gray-800 border-gray-700">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="text-sm text-gray-300">Max Score</label>
                        <Input
                          type="number"
          ...
          <div className="space-y-4">
            {assignments.map((assignment) => (
              <div key={assignment.id} className="bg-gray-800 border border-gray-700 rounded-lg p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
          ...
          <div className="flex justify-between items-center">
            ...
            {canManageContent && (
              <Dialog open={showLectureDialog} onOpenChange={setShowLectureDialog}>
                ...
                <DialogContent className="bg-gray-800 border-gray-700">
                  <DialogHeader>
                    <DialogTitle className="text-white">Upload Lecture</DialogTitle>
                  </DialogHeader>
                  <div className="space-y-4">
                    <Input
                    ...
                    <div className="border-2 border-dashed border-gray-600 rounded-lg p-4 text-center">
                      ...
                      <p className="text-sm text-gray-500">PDF, PPT, MP4, etc.</p>
                    </div>
                  </div>
                  <DialogFooter>
                    <Button onClick={handleCreateLecture} className="bg-purple-600 hover:bg-purple-700">
                      Upload Lecture
                    </Button>
                  </DialogFooter>
                </DialogContent>
          ...
          <div className="space-y-4">
            {lectures.map((lecture) => (
              <div key={lecture.id} className="bg-gray-800 border border-gray-700 rounded-lg p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="text-xl font-semibold text-white mb-2">{lecture.title}</h3>
                    <p className="text-gray-300 mb-4">{lecture.description}</p>
                    <div className="flex gap-2">
                      {lecture.materials.map((material, index) => (
                        <Button key={index} size="sm" variant="outline" className="text-blue-400 border-blue-400">
        ...
        <TabsContent value="ai" className="space-y-4">
          <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
            <div className="text-center py-12">
              ...
              <p className="text-gray-400">Your intelligent course assistant is coming soon!</p>
            </div>
          </div>
        </TabsContent>
          ...
          <div className="flex justify-between items-center">
            ...
            {canManagePeople && (
              <Dialog open={showAddPersonDialog} onOpenChange={setShowAddPersonDialog}>
                ...
                <DialogContent className="bg-gray-800 border-gray-700">
                  <DialogHeader>
                    <DialogTitle className="text-white">Add Person to Course</DialogTitle>
                  </DialogHeader>
                  <div className="space-y-4">
                    <Input
                      placeholder="Email address"
                      value={personToAdd.email}
                      onChange={(e) => setPersonToAdd({ ...personToAdd, email: e.target.value })}
                      className="bg-gray-700 border-gray-600 text-white"
                    />
                    <select
          ...
          {course?.tas && course.tas.length > 0 && (
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-6">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-lg font-semibold text-white">Teaching Assistants</h3>
              </div>
              <div className="space-y-3">
                {course.tas.map((ta) => (
                  <div key={ta.id} className="flex items-center justify-between p-3 bg-gray-700 rounded-lg">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-10 w-10">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {canManagePeople && (
                      <Button 
                        size="sm" 
                        variant="destructive"
                        onClick={() => handleRemovePerson(student.id, student.name)}
                      >
                        <UserMinus className="h-4 w-4" />
                      </Button>
                    )}
